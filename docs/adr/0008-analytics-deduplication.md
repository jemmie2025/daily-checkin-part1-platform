...
ClickHouse stores rows in `ReplicatedReplacingMergeTree`, while every Grafana
...
history while the Grafana identity remains denied on all physical tables.
...
- The raw table remains unavailable to the Grafana role.

...
|---|---|---|---|
| SUB-1 APISIX policies | Part 1 | Existing-platform route/plugin overlay, custom plugin, deterministic renderer | Policy, fail-closed, and latency-plan tests |
| SUB-1 Vault bindings | Part 1 | Exact-path policy and workload-role overlays for existing Vault/Nomad | Policy, isolation, rotation, and secret-scan tests |
...
| ClickHouse/Grafana metrics | Part 1 | Privacy-safe facts, reporting views, and six-panel Grafana dashboard | Source-to-panel reconciliation and asset tests |
| Violation/DLQ analytics | Part 1 | Minimized versioned facts inserted through the analytics workflow | Schema, privacy, deduplication, and lag tests |
...
| Gitleaks | Part 1 security gate | Full-history secret scan using reviewed configuration | Pull-request and release scan evidence |
| Strix | Part 1 security gate | Authorised non-interactive scan of repository or staging target | Reviewed report and critical-finding gate |
